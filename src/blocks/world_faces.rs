...
impl WorldFaces{
    pub fn new(size:WorldSize, device:&Device)->Result<Self,failure::Error>{
        Ok(Self{ chunks: std::iter::repeat_with(||ChunkFaces::new(device)).take(size.total_chunks()).collect::<Result<Vec<ChunkFaces>,failure::Error>>()?, size })
    }
    ...
    pub(crate) fn push_block(&mut self, x: usize, y: usize, z: usize, ort: FaceOrientation, block: Block) -> Result<bool, failure::Error> {
    ...
    pub(crate) fn change_block_textures(&mut self, x: usize, y: usize, z: usize, new_block: Block) -> Result<(), failure::Error> {

...
impl WorldBlocks {
    ...
    pub fn fill(&mut self, from_x: usize, from_y: usize, from_z: usize, width: usize, height: usize, depth: usize, block: Block) {
    ...
    pub fn fill_column(&mut self, x: usize, from_y: usize, z: usize, height: usize, block: Block) {
        self.fill_column_to(x,from_y,z,from_y+height,block)
    }

    pub fn fill_column_to(&mut self, x: usize, from_y: usize, z: usize, to_y: usize, block: Block) {
        for y in from_y..to_y {
            self.set_block(x, y, z, block);
        }
    }

    pub fn outline(&mut self, from_x: usize, from_y: usize, from_z: usize, width: usize, height: usize, depth: usize, block: Block) {
        self.fill(from_x, from_y, from_z, width, 1, depth, block);
        if height > 1 {
            self.fill(from_x, from_y + height - 1, from_z, width, 1, depth, block);
            if height > 2 {
                for y in from_y + 1..(from_y + height - 1) {
                    if width > 0 {
                        for z in from_z..(from_z + depth) {
                            self.set_block(from_x, y, z, block);
                            self.set_block(from_x + width - 1, y, z, block);
                        }
                    }
                    if depth > 2 {
                        for x in from_x..(from_x + width) {
                            self.set_block(x, y, from_z, block);
                            self.set_block(x, y, from_z + depth - 1, block);
                        }
                    }
                }
            }
    ...
    pub fn fill_level(&mut self, from_y: usize, height: usize, block: Block) {
        self.fill(0, from_y, 0, self.size().world_width(), height, self.size().world_depth(), block)
    }

    pub fn replace(&mut self, from_x: usize, from_y: usize, from_z: usize, width: usize, height: usize, depth: usize, old_block: Block, new_block: Block) {
        for x in from_x..(from_x + width) {
            for y in from_y..(from_y + height) {
                for z in from_z..(from_z + depth) {
    ...
    pub fn heightmap(&mut self, filler_block: Block, height_at: impl Fn(usize, usize) -> usize) {
        for x in 0..self.size().world_width() {
            for z in 0..self.size().world_depth() {
                for y in 0..height_at(x, z) {
                    self.set_block(x, y, z, filler_block)
                }
            }
        }
    }

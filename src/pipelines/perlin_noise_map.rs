...
impl PerlinNoiseMap {
    ...
    pub fn new(world_size: WorldSize, scale:f32, mean: f32, span: f32) -> Self {
        ...
        let chunk_vals: Vec<f32> = (0..size_with_margins.0 * size_with_margins.1).map(|_| mean - span/2. + f32::random()*span).collect();
        for x in 0..world_size.world_width() {
            for z in 0..world_size.world_depth() {
                ...
                let val = fraction_x.smoothstep_between(val_left, val_right) as usize;
    ...
    pub fn val(&self, x: usize, z: usize) -> usize {
